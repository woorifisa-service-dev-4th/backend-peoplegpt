...
//    }
    ...
    void should_Return_Post_Detail() {
        ...
        Post post = new Post(
                1,  // postId
                1,  // userId
                "Spring Security를 활용한 OAuth2 인증 구현",
                "OAuth2를 적용하여 보안성을 강화하는 방법을 공유합니다.",
                Category.QNA,
                ClassFilter.AI,
                Tag.PROJECT,
                DataStatus.ACTIVE,
                LocalDateTime.parse("2025-02-11T10:00:00")
        );


        when(postRepository.findPostByPostId(postId)).thenReturn(post);

        // When
        PostDetailResponse postDetailResponse = postServiceImpl.getPostByPostId(postId);

        // Then
        assertAll(
                () -> assertEquals(post.getPostId(), postDetailResponse.getPostId(), "postID가 일치하지 않습니다."),
                () -> assertEquals(post.getUserId(), postDetailResponse.getUserId(), "userID가 일치하지 않습니다."),
                () -> assertEquals(post.getTitle(), postDetailResponse.getTitle(), "제목이 일치하지 않습니다."),
                () -> assertEquals(post.getContent(), postDetailResponse.getContent(), "내용이 일치하지 않습니다."),
                () -> assertEquals(post.getCategory(), postDetailResponse.getCategory(), "카테고리가 일치하지 않습니다."),
                () -> assertEquals(post.getFilter(), postDetailResponse.getFilter(), "필터가 일치하지 않습니다."),
                () -> assertEquals(post.getTag(), postDetailResponse.getTag(), "태그가 일치하지 않습니다."),
                () -> assertEquals(post.getStatus(), postDetailResponse.getStatus(), "상태가 일치하지 않습니다."),
                () -> assertEquals(post.getCreatedAt(), postDetailResponse.getCreatedAt(), "생성 시간이 일치하지 않습니다.")
        );

        // Verify that the repository method was called once
        verify(postRepository, times(1)).findPostByPostId(postId);
    }
}
